...
const ENROLLMENT_ROLE_LABELS: Record<EnrollmentRole, string> = {
  monitor: "班长", attendance_assistant: "考勤员", group_leader: "组长", charity: "慈善", dharma_light: "传灯",
...
function classPermissionLabel(permission: ClassSummary["permission"], isAdmin = false) {
  if (isAdmin || permission === "admin") return "管理员";
  if (permission === "counselor") return "辅导员";
  if (permission === "attendance_assistant") return "考勤员";
  return "班长";
}

function accountIdentityLabel(user: CurrentUser) {
  if (user.isAdmin) return "系统管理员";
  const identities = new Set<string>();
  if (user.canCounsel || user.classAccesses.some((item) => item.permission === "counselor")) identities.add("辅导员");
  if (user.classAccesses.some((item) => item.permission === "monitor")) identities.add("班长");
  if (user.classAccesses.some((item) => item.permission === "attendance_assistant")) identities.add("考勤员");
  return [...identities].join("、") || "普通账号";
}
...
function lessonFromRaw(raw: Record<string, unknown>): Lesson {
  const classStudyDueDate = dateOnly(raw.classStudyDueDate ?? raw.dueDate);
  const hasRecordedAttendance = raw.hasRecordedAttendance == null
    ? raw.scheduleEditable === false
    : Boolean(raw.hasRecordedAttendance);
  const scheduleLocked = raw.scheduleLocked == null
    ? hasRecordedAttendance && classStudyDueDate < shanghaiTodayClient()
    : Boolean(raw.scheduleLocked);
  return {
    ...
    scheduleLocked,
    scheduleEditable: raw.scheduleEditable == null ? !scheduleLocked : Boolean(raw.scheduleEditable),
    lockedForMonitor: Boolean(raw.lockedForMonitor),
    status: raw.status as Lesson["status"]
  };
}

function adjustableScheduleSuffix(lessons: Lesson[]): Lesson[] {
  const ordered = [...lessons].sort((left, right) => left.lessonNumber - right.lessonNumber);
  let lastUneditableIndex = -1;
  ordered.forEach((lesson, index) => {
    if (!lesson.scheduleEditable) lastUneditableIndex = index;
  });
  return ordered.slice(lastUneditableIndex + 1);
}
    ...
    <main className="login-page">
      ...
      <section className="login-card">
        ...
        <div>
          ...
          <h2>登录班级共修管理系统</h2>
          <p>管理员使用账号；辅导员、班长和考勤员可使用手机号或分配的拼音账号登录。</p>
        </div>
...
function AppShell({ user, classes, currentClass, onSelectClass, onLogout, children }: ShellProps) {
  ...
  const attendanceAssistant = currentClass?.permission === "attendance_assistant";
  const navItems = [
    ...
    { path: "/lessons", label: "课表安排", icon: CalendarDays, needsClass: true, schedule: true },
  ...
  ].filter((item) => (!item.manager || manager) && (!item.schedule || !attendanceAssistant));
    ...
    <div className="app-shell">
      ...
      <aside id="main-navigation" className={`sidebar ${mobileOpen ? "open" : ""}`} aria-label="主导航">
        ...
        <div className="sidebar-foot">
          ...
          <div className="user-meta"><strong>{user.displayName}</strong><span>{currentClass ? classPermissionLabel(currentClass.permission, user.isAdmin) : accountIdentityLabel(user)}</span></div>
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="topbar-right">
            {currentClass && <span className={`role-badge ${currentClass.permission ?? (user.isAdmin ? "admin" : "")}`}>{classPermissionLabel(currentClass.permission, user.isAdmin)}</span>}
  ...
  return <main className="page">
    ...
    <div className="settings-grid">
      ...
      <section className="panel form-stack">
        ...
        <dl className="profile-facts"><div><dt>登录账号</dt><dd>{user.isAdmin ? "admin" : user.username || "—"}</dd></div><div><dt>账号身份</dt><dd>{accountIdentityLabel(user)}</dd></div></dl>
...
function ClassHub({ user, classes, onRefresh, onSelect }: { user: CurrentUser; classes: ClassSummary[]; onRefresh: () => Promise<void>; onSelect: (id: number) => void }) {
  ...
  async function setClassStopped(item: ClassSummary, stopped: boolean) {
    ...
    if (!window.confirm(`确定${action}班级“${item.name}”吗？${stopped ? "班长和考勤员将立即停止访问，历史数据会保留。" : "班级将恢复日常使用。"}`)) return;
    setLoading(true);
    try {
          ...
          {visibleClasses.map((item) => (
            <article className={`class-card ${item.archived ? "stopped" : ""}`} key={item.id}>
              ...
              <div className="class-card-body">
                <div className="card-title-row"><h2>{item.name}</h2><span className="soft-badge">{classPermissionLabel(item.permission, user.isAdmin)}</span></div>
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [reportData, lessonsData, studentsData] = await Promise.all([
        ...
        attendanceAssistant ? Promise.resolve(null) : apiJson<unknown>(`/api/classes/${currentClass.id}/students`)
      ]);
      ...
      setStudents(studentsData == null ? [] : asList<Record<string, unknown>>(studentsData, "students", "items").map(studentFromRaw));
    } catch (error) {
      setNotice({ tone: "error", text: errorText(error) });
    } finally { setLoading(false); }
  }, [attendanceAssistant, currentClass.id]);
    ...
    <main className="page">
      ...
      <section className="hero-card">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><button className="primary" onClick={() => go("/attendance")} disabled={!currentLesson}><ClipboardCheck size={17} /> 去登记考勤</button>{!attendanceAssistant && <button className="glass-button" onClick={() => go("/lessons")}><CalendarDays size={17} /> 查看课表</button>}</div>
        </div>
        <div className="hero-art"><CloudSun size={68} /><span>{attendanceAssistant ? currentClass.studentCount ?? 0 : activeStudents.length}</span><small>在册学员</small></div>
      </section>
      <section>
...
function StudentEditor({ classId, student, groups, isAdmin, onClose, onSaved }: { classId: number; student: Student | null; groups: Group[]; isAdmin: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [identities, setIdentities] = useState<EnrollmentRole[]>(student?.identities?.filter((role) => !["monitor", "attendance_assistant", "student"].includes(role)) ?? []);
  const [currentPassword, setCurrentPassword] = useState("");
  const [loading, setLoading] = useState(false);
  const [notice, setNotice] = useState<NoticeState>(null);
  ...
  return <Modal title={student ? "编辑学员" : "新增学员"} subtitle="新增和转组从下一课起生效，历史课次不会改变。" onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
      ...
      <fieldset className="identity-field"><legend>班级身份（可多选）</legend><div className="checkbox-grid">{EDITABLE_ROLE_OPTIONS.map((option) => <label key={option.value}><input type="checkbox" checked={identities.includes(option.value)} onChange={(event) => setIdentities((current) => event.target.checked ? [...current, option.value] : current.filter((role) => role !== option.value))} /><span>{option.label}</span></label>)}</div><small>“学员”默认显示；“班长”和“考勤员”由班级设置统一管理。每组只能设置一名组长。</small></fieldset>
...
function LessonEditor({ classId, lesson, onClose, onSaved }: { classId: number; lesson: Lesson; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    event.preventDefault(); setLoading(true);
    try {
      await apiJson(`/api/classes/${classId}/lessons/${lesson.id}`, {
        method: "PATCH",
        body: JSON.stringify({ title, ...(!lesson.hasRecordedAttendance ? { lessonType } : {}), classStudyDueDate })
      });
      await onSaved(); onClose();
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  }
  const editorSubtitle = lesson.scheduleLocked && lesson.scheduleEditable
    ? "本课底层状态已锁定，管理员仍可修改课名和日期；已有考勤会保留，课次类型不可更改。"
    : lesson.hasRecordedAttendance
      ? "本课已有考勤但尚未过整课截止日，仍可修改课名和日期；考勤会保留，课次类型不可更改。"
      : "本课没有考勤，即使日期已过也可以修改；修改日期会让本课及后续可调整的安排整体顺延。";
  return <Modal title={`编辑第 ${lesson.lessonNumber} 个课次`} subtitle={editorSubtitle} onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
      <label>课次名称<input value={title} onChange={(e) => setTitle(e.target.value)} required /></label>
      <label>课次类型<select value={lessonType} onChange={(e) => setLessonType(e.target.value as Lesson["lessonType"])} disabled={lesson.hasRecordedAttendance}><option value="regular">普通课</option><option value="review">复习课（导图/提纲自动不需要）</option></select>{lesson.hasRecordedAttendance && <small>本课已有考勤，为避免改变导图/提纲适用规则，课次类型不可更改。</small>}</label>
...
function InsertLesson({ classId, lessons, seriesKey, onClose, onSaved }: {
  ...
  const insertableLessons = adjustableScheduleSuffix(lessons);
  const canOverrideLocked = insertableLessons.some((lesson) => lesson.scheduleLocked && lesson.scheduleEditable);
  const [beforeLessonId, setBeforeLessonId] = useState(String(insertableLessons[0]?.id ?? ""));
  const target = insertableLessons.find((lesson) => String(lesson.id) === beforeLessonId) ?? insertableLessons[0];
  ...
  function chooseTarget(value: string) {
    ...
    const nextTarget = insertableLessons.find((lesson) => String(lesson.id) === value);
  ...
  return <Modal title="插入课次" subtitle={canOverrideLocked ? "管理员可从全课表选择插入位置；原插入位置及后续课次会整体后移，已有考勤随原课次保留。" : "只在最后一个已锁定课次之后插入；原插入位置及后续可调整课次会整体后移，已有考勤随原课次保留。"} onClose={onClose}>
    {!target ? <EmptyState title="没有可插入的位置" detail="当前权限下没有可调整课次；如需继续排课，请使用追加课次。" /> : <form className="form-stack" onSubmit={submit}>
      <label>插入位置<select value={beforeLessonId} onChange={(event) => chooseTarget(event.target.value)}>{insertableLessons.map((lesson) => <option key={lesson.id} value={lesson.id}>第 {lesson.lessonNumber} 个课次之前 · {lesson.title}</option>)}</select><small>{canOverrideLocked ? "管理员可越过底层锁定状态；" : "其他角色只能调整最后一个已锁定课次之后的范围；"}移动课次不会清空已有考勤。</small></label>
...
function RebuildFutureSchedule({ classId, lessons, currentClass, canSync, onClose, onSaved }: {
  ...
  const replaceableLessons = adjustableScheduleSuffix(lessons);
  const replaceableIds = new Set(replaceableLessons.map((lesson) => lesson.id));
  const preservedLessons = lessons.filter((lesson) => !replaceableIds.has(lesson.id));
  const preservedCount = lessons.length - replaceableLessons.length;
  const preservedLockedCount = preservedLessons.filter((lesson) => lesson.scheduleLocked).length;
  const affectedAttendanceCount = replaceableLessons.filter((lesson) => lesson.hasRecordedAttendance).length;
  const protectedAdjustableCount = preservedCount - preservedLockedCount;
  const canOverrideLocked = replaceableLessons.some((lesson) => lesson.scheduleLocked && lesson.scheduleEditable);
  const [firstDueDate, setFirstDueDate] = useState(replaceableLessons[0]?.classStudyDueDate ?? "");
  const [count, setCount] = useState(replaceableLessons.length || 24);
  ...
  const [startPosition, setStartPosition] = useState(replaceableLessons[0]?.coursePosition ?? ((currentClass.courseStartPosition ?? 1) + preservedCount));
  ...
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!replaceableLessons.length) return;
    const protectedDetail = protectedAdjustableCount > 0 ? `（含 ${protectedAdjustableCount} 个位于最后锁定课次之前的可调整课次）` : "";
    if (!window.confirm(`将保留前面的 ${preservedCount} 个课次${protectedDetail}，替换其后的 ${replaceableLessons.length} 个可调整课次，并生成 ${count} 个新课次。确定继续吗？`)) return;
    if (affectedAttendanceCount > 0 && !window.confirm(`替换范围内有 ${affectedAttendanceCount} 个课次已经填写考勤。继续操作会永久清空这些课次的全部考勤及变更历史，且无法恢复。\n\n请再次确认是否继续。`)) return;
    setLoading(true); setNotice(null);
    try {
      await apiJson(`/api/classes/${classId}/schedule/rebuild-future`, {
        method: "POST",
        body: JSON.stringify({ firstClassStudyDueDate: firstDueDate, count, cadenceMode, seriesKey, startPosition, round, confirmDiscardAttendance: affectedAttendanceCount > 0 })
      });
      await onSaved(); onClose();
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  ...
  return <Modal title="重新生成可调整课表" subtitle={canOverrideLocked ? "管理员可重新生成全课表，包括底层已锁定课次；替换范围内已有的考勤会在二次确认后清空。" : "最后一个已锁定课次之后，历史、当前或未来课次都可重新生成；替换范围内已有的考勤会在二次确认后清空。"} onClose={onClose}>
    {!replaceableLessons.length ? <EmptyState title="没有可重新生成的课次" detail="当前权限下没有可调整课次；如需继续排课，请使用追加课次。" /> : <form className="form-stack" onSubmit={submit}>
      <div className="callout"><strong>变更预览</strong><span>{preservedCount > 0 ? `保留前面的 ${preservedCount} 个课次（${preservedLockedCount} 个已锁定${protectedAdjustableCount > 0 ? `，${protectedAdjustableCount} 个位于最后不可调整课次之前` : ""}）` : "不保留原课次"} · 替换 {replaceableLessons.length} 个可调整课次{affectedAttendanceCount > 0 ? `（其中 ${affectedAttendanceCount} 个已有考勤，将被清空）` : ""} · 新生成 {count} 个课次</span></div>
      ...
      <Notice notice={notice} />
      <div className="modal-actions"><button type="button" className="ghost" onClick={onClose}>取消</button><button className="primary" disabled={loading}>{loading ? "重新生成中..." : "重新生成可调整课表"}</button></div>
    </form>}
  </Modal>;
}
...
function GenerateSchedule({ classId, defaultMode, hasLessons, canSync, defaultSeriesKey, defaultRound, defaultStartPosition, sourceProgress, onClose, onSaved }: { classId: number; defaultMode: CadenceMode; hasLessons: boolean; canSync: boolean; defaultSeriesKey?: string | null; defaultRound?: number; defaultStartPosition?: number; sourceProgress?: string | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [notice, setNotice] = useState<NoticeState>(null);
  const [loading, setLoading] = useState(false);
  const loadCatalog = useCallback(async (currentSelection?: { seriesKey: string; startPosition: number }) => {
    const data = await apiJson<unknown>("/api/course-catalog");
    const options = asList<Record<string, unknown>>(data, "series", "items").map((entry) => ({
      key: String(entry.key), displayName: String(entry.displayName), syncedAt: entry.syncedAt == null ? null : String(entry.syncedAt),
      items: asList<Record<string, unknown>>(entry.items, "items").map((item) => ({ position: Number(item.position), title: String(item.title), lessonType: String(item.lessonType) }))
    ...
    const currentSeries = currentSelection
      ? options.find((entry) => entry.key === currentSelection.seriesKey)
      : undefined;
    if (currentSeries && currentSelection) {
      setSeriesKey(currentSeries.key);
      setStartPosition(currentSeries.items.some((item) => item.position === currentSelection.startPosition)
        ? currentSelection.startPosition
        : currentSeries.items[0]?.position ?? 1);
      return;
    }
    const preferredIndex = options.findIndex((entry) => entry.key === (defaultSeriesKey ?? "wisdom_life"));
    const preferred = options[preferredIndex >= 0 ? preferredIndex : 0];
    const desiredPosition = defaultStartPosition ?? 1;
    if (preferred.items.some((item) => item.position === desiredPosition)) {
      setSeriesKey(preferred.key);
      setStartPosition(desiredPosition);
      return;
    }
    const fallback = hasLessons && preferredIndex >= 0
      ? options[preferredIndex + 1] ?? preferred
      : preferred;
    setSeriesKey(fallback.key);
    setStartPosition(fallback.items[0]?.position ?? 1);
  }, [defaultSeriesKey, defaultStartPosition, hasLessons]);
  useEffect(() => { void loadCatalog().catch((error) => setNotice({ tone: "error", text: errorText(error) })); }, [loadCatalog]);
  ...
  async function syncCatalog() {
    ...
    try {
      const result = await apiJson<{ seriesCount: number; itemCount: number }>("/api/admin/course-catalog/sync", { method: "POST" });
      await loadCatalog({ seriesKey, startPosition });
      setNotice({ tone: "success", text: `官方课程目录已更新：${result.seriesCount} 个体系，${result.itemCount} 个课次。` });
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  }
  async function submit(event: FormEvent) {
    event.preventDefault(); setLoading(true);
    try {
      const body = hasLessons
        ? { count, course: { seriesKey, startPosition, round } }
        : { firstClassStudyDueDate: firstDueDate, count, cadenceMode, seriesKey, startPosition, round };
      await apiJson(hasLessons ? `/api/classes/${classId}/lessons/append` : `/api/classes/${classId}/schedule/generate`, { method: "POST", body: JSON.stringify(body) });
      await onSaved(); onClose();
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  }
  const selectedSeries = series.find((entry) => entry.key === seriesKey);
  const selectedStartExists = Boolean(selectedSeries?.items.some((item) => item.position === startPosition));
  return <Modal title={hasLessons ? "追加课次" : "生成学习课表"} subtitle={hasLessons ? "从现有最后截止日继续排期；可接着当前课程，也可切换到下一套课程。" : "选择课程体系、遍数和起始课；复习课会自动识别。"} onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
      <div className="form-grid two"><label>{hasLessons ? "追加哪套课程" : "课程体系"}<select value={seriesKey} onChange={(event) => { const next = series.find((entry) => entry.key === event.target.value); setSeriesKey(event.target.value); setStartPosition(next?.items[0]?.position ?? 1); }}>{series.map((entry) => <option value={entry.key} key={entry.key}>{entry.displayName}</option>)}</select></label><label>第几遍<input type="number" min={1} max={20} value={round} onChange={(event) => setRound(Number(event.target.value))} /></label></div>
      {!hasLessons && sourceProgress && <div className="callout"><strong>原表课程进度</strong><span>{sourceProgress}</span></div>}
      <label>从哪一课开始<select value={startPosition} onChange={(event) => setStartPosition(Number(event.target.value))}>{selectedSeries?.items.map((item) => <option key={item.position} value={item.position}>{item.title}</option>)}</select><small>{hasLessons ? "已有课次和考勤保持不变，新课程从现有最后截止日之后开始。" : "起点之前的课不会生成，也不会进入完成率统计。"}</small></label>
      ...
      <Notice notice={notice} />
      <div className="modal-actions"><button type="button" className="ghost" onClick={onClose}>取消</button><button className="primary" disabled={loading || !selectedStartExists}>{loading ? (hasLessons ? "追加中..." : "生成中...") : `${hasLessons ? "追加" : "生成"} ${count} 课`}</button></div>
    </form>
  </Modal>;
}
...
function AddBreak({ classId, onClose, onSaved }: { classId: number; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal title="插入放假 / 暂停周" subtitle="该周不考勤、不统计；后续可调整课次整体顺延，已有考勤随原课次保留，不会被清空。" onClose={onClose}>
...
function LessonsPage({ currentClass, isAdmin, onClassRefresh }: { currentClass: ClassSummary; isAdmin: boolean; onClassRefresh: () => Promise<void> }) {
  ...
  const adjustableSuffix = adjustableScheduleSuffix(lessons);
  const lastCatalogLesson = [...lessons].reverse().find((lesson) => lesson.coursePosition != null);
  const nextCoursePosition = lastCatalogLesson?.coursePosition == null
    ? currentClass.courseStartPosition ?? 1
    : lastCatalogLesson.coursePosition + 1;

  async function removeLesson(lesson: Lesson) {
    if (!lesson.scheduleEditable) return;
    const futureLesson = !lesson.started;
    const overrideNotice = lesson.scheduleLocked ? "本课底层状态已锁定，管理员正在越过锁定删除。\n" : "";
    const impact = futureLesson
      ? "后续未来课次编号会依次前移，原有日期保持不变。"
      : "这是已开始或历史课次；删除后会保留后续课次原编号和原日期，课次编号可能出现空号。";
    if (!window.confirm(`${overrideNotice}确定删除第 ${lesson.lessonNumber} 个课次“${lesson.title}”吗？\n\n${impact}`)) return;
    if (lesson.hasRecordedAttendance && !window.confirm("本课已经填写考勤。继续删除会永久清空本课全部学员的考勤及变更历史，且无法恢复。\n\n请再次确认是否删除。")) return;
    setLoading(true); setNotice(null);
    try {
      await apiJson(`/api/classes/${currentClass.id}/lessons/${lesson.id}`, {
        method: "DELETE",
        body: JSON.stringify({ confirmDiscardAttendance: Boolean(lesson.hasRecordedAttendance) })
      });
      setNotice({ tone: "success", text: futureLesson
        ? `已删除“${lesson.title}”${lesson.hasRecordedAttendance ? "并清空本课考勤" : ""}，后续未来课次已重新编号`
        : `已删除“${lesson.title}”${lesson.hasRecordedAttendance ? "并清空本课考勤" : ""}，后续课次的原编号和日期均已保留` });
  ...
  return <main className="page">
    <PageHeader eyebrow="SCHEDULE" title="课表安排" description={isAdmin ? "管理员可调整全部课次，包括底层已锁定课次；已有考勤的删除或重建需要二次确认。锁定条件仍是“已有考勤且已过整课截止日”，截止日当天仍可调整。" : "仅“已有考勤且已过整课截止日”的课次锁定，截止日当天仍可调整；无考勤课次即使过期也可编辑或删除。"} actions={<>{lessons.length > 0 && <><button className="secondary" onClick={() => setShowBreak(true)}><CloudSun size={17} /> 放假 / 暂停周</button><button className="secondary" onClick={() => setShowInsert(true)} disabled={!adjustableSuffix.length} title={adjustableSuffix.length ? (isAdmin ? "管理员可从全课表插入" : "在最后一个已锁定课次之后插入") : "当前权限下没有可插入位置，请使用追加课次"}><Plus size={17} /> 插入课次</button><button className="secondary" onClick={() => setShowRebuild(true)} disabled={!adjustableSuffix.length} title={adjustableSuffix.length ? (isAdmin ? "管理员可重新生成全课表" : "重新生成最后一个已锁定课次之后的课表") : "没有可重新生成的课次"}><RefreshCw size={17} /> 重新生成可调整课表</button></>}<button className="primary" onClick={() => setShowGenerate(true)}><CalendarDays size={17} /> {lessons.length ? "追加课次" : "生成课表"}</button></>} />
    <Notice notice={notice} onClose={() => setNotice(null)} />
    ...
    {showGenerate && <GenerateSchedule classId={currentClass.id} defaultMode={currentClass.cadenceMode ?? "same_week"} hasLessons={lessons.length > 0} canSync={isAdmin} defaultSeriesKey={currentClass.courseSeriesKey} defaultRound={currentClass.courseRound} defaultStartPosition={lessons.length > 0 ? nextCoursePosition : currentClass.courseStartPosition} sourceProgress={currentClass.sourceProgress} onClose={() => setShowGenerate(false)} onSaved={reloadScheduleAndClass} />}
...
function attendanceAssistantFromRaw(raw: Record<string, unknown>): AttendanceAssistant {
  return {
    enrollmentId: Number(raw.enrollmentId ?? raw.studentId ?? raw.id),
    userId: Number(raw.userId ?? raw.accountId),
    name: String(raw.name ?? ""),
    dharmaName: raw.dharmaName == null ? null : String(raw.dharmaName),
    phone: raw.phone == null ? null : String(raw.phone),
    username: String(raw.username ?? raw.loginIdentifier ?? "")
  };
}
...
function SettingsPage({ user, currentClass, onRefresh }: { user: CurrentUser; currentClass: ClassSummary; onRefresh: () => Promise<void> }) {
  ...
  const loadAttendanceAssistants = useCallback(async () => {
    const data = await apiJson<unknown>(`/api/classes/${currentClass.id}/attendance-assistants`);
    setAttendanceAssistants(asList<Record<string, unknown>>(data, "assistants", "items").map(attendanceAssistantFromRaw));
  }, [currentClass.id]);
  ...
  useEffect(() => {
    ...
    setAssistantId(""); setAssistantUsername("");
    void Promise.all([
  ...
  }, [currentClass, loadAttendanceAssistants, user.isAdmin]);
  ...
  async function assignMonitor() {
    ...
    try {
      ...
      const temporaryPassword = String(result.temporaryPassword ?? (result.account as Record<string, unknown> | undefined)?.temporaryPassword ?? "");
      const loginIdentifier = String(result.loginIdentifier ?? result.username ?? "");
      await Promise.all([onRefresh(), loadAttendanceAssistants()]);
  ...
  async function assignAttendanceAssistant() {
    if (!assistantId) {
      setNotice({ tone: "info", text: "请先选择一位正常学员" });
      return;
    }
    setLoading(true);
    try {
      const result = await apiJson<Record<string, unknown>>(`/api/classes/${currentClass.id}/attendance-assistants`, {
        method: "POST",
        body: JSON.stringify({ studentId: Number(assistantId), username: assistantUsername.trim() || undefined })
      });
      const temporaryPassword = String(result.temporaryPassword ?? (result.account as Record<string, unknown> | undefined)?.temporaryPassword ?? "");
      const loginIdentifier = String(result.loginIdentifier ?? result.username ?? (result.account as Record<string, unknown> | undefined)?.username ?? "");
      const selected = students.find((item) => item.id === Number(assistantId));
      await loadAttendanceAssistants();
      setAssistantId(""); setAssistantUsername("");
      setNotice({
        tone: "success",
        text: temporaryPassword
          ? `考勤员已设置：登录账号 ${loginIdentifier}，临时密码 ${temporaryPassword}`
          : `“${selected?.name || selected?.dharmaName || "所选学员"}”已获得考勤员权限${loginIdentifier ? `，登录账号 ${loginIdentifier}` : ""}`
      });
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  }

  async function removeAttendanceAssistant(assistant: AttendanceAssistant) {
    if (!window.confirm(`确定取消“${assistant.name || assistant.dharmaName || "当前考勤员"}”的考勤员权限吗？取消后会立即停止以此身份访问本班。`)) return;
    setLoading(true);
    try {
      await apiJson(`/api/classes/${currentClass.id}/attendance-assistants/${assistant.enrollmentId}`, { method: "DELETE" });
      await loadAttendanceAssistants();
      setNotice({ tone: "success", text: "考勤员权限已取消，学员身份和历史修改记录仍会保留" });
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  }

  async function resetAttendanceAssistantPassword(assistant: AttendanceAssistant) {
    if (!window.confirm(`确定为“${assistant.name || assistant.dharmaName || "当前考勤员"}”生成新的临时密码吗？`)) return;
    setLoading(true);
    try {
      const result = await apiJson<{ temporaryPassword: string }>(`/api/classes/${currentClass.id}/attendance-assistants/${assistant.enrollmentId}/reset-password`, { method: "POST" });
      setNotice({ tone: "success", text: `考勤员 ${assistant.name || assistant.dharmaName} 的临时密码已重置：${result.temporaryPassword}；首次登录必须修改。` });
    } catch (error) { setNotice({ tone: "error", text: errorText(error) }); }
    finally { setLoading(false); }
  }
  ...
  async function toggleClassStatus() {
    ...
    if (!window.confirm(restoring ? `确定恢复“${currentClass.name}”吗？` : `确定停用“${currentClass.name}”吗？班长和考勤员将停止访问，历史数据仍会保留。`)) return;
    setLoading(true);
    try {
  ...
  return <main className="page">
    ...
    <div className="settings-grid">
      ...
      <section className="panel form-stack">
        ...
        <div className="permission-note"><strong>班长可以做什么？</strong><span>登记本班三项考勤、查看本班统计，并管理尚未开始的课表；不能查看手机号和备注，也不能维护学员、小组或班级设置。</span></div>
      </section>
      <section className="panel form-stack attendance-assistant-settings">
        <div className="panel-head"><div><h2>考勤员账号</h2><p>可从正常学员中指定多位考勤协助人员。</p></div><ClipboardCheck size={20} /></div>
        {attendanceAssistants.length ? <div className="assistant-account-list">{attendanceAssistants.map((assistant) => <article className="assistant-account" key={assistant.enrollmentId}>
          <div className="person-cell"><div className="mini-avatar">{(assistant.name || assistant.dharmaName || "考").slice(0, 1)}</div><span><strong>{assistant.name || assistant.dharmaName}</strong><small>{assistant.dharmaName && assistant.dharmaName !== assistant.name ? `${assistant.dharmaName} · ` : ""}账号：{assistant.username || "—"}</small></span><span className="role-badge attendance_assistant">考勤员</span></div>
          <div className="assistant-account-actions"><button type="button" className="secondary" onClick={() => void resetAttendanceAssistantPassword(assistant)} disabled={loading}>重置密码</button><button type="button" className="ghost danger" onClick={() => void removeAttendanceAssistant(assistant)} disabled={loading}>取消权限</button></div>
        </article>)}</div> : <p className="assistant-empty">本班暂未设置考勤员。</p>}
        <label>新增考勤员<select value={assistantId} onChange={(event) => { setAssistantId(event.target.value); setAssistantUsername(""); }}><option value="">请选择正常学员</option>{students.filter((student) => student.active !== false && student.status === "normal" && !attendanceAssistants.some((assistant) => assistant.enrollmentId === student.id)).map((student) => <option key={student.id} value={student.id}>{student.name || student.dharmaName}{student.name && student.dharmaName && student.name !== student.dharmaName ? `（${student.dharmaName}）` : ""} · {student.groupName}{student.phone ? "" : " · 无手机号"}</option>)}</select><small>已有账号时直接增加权限；没有账号时会生成一次性临时密码。</small></label>
        {assistantId && !students.find((item) => item.id === Number(assistantId))?.phone && <label>考勤员登录账号（选填）<input value={assistantUsername} onChange={(event) => setAssistantUsername(event.target.value)} autoCapitalize="none" placeholder="留空则根据姓名或法名自动生成" /><small>可填写希望使用的拼音账号；重名时系统会提示修改。</small></label>}
        <button type="button" className="primary align-start" onClick={() => void assignAttendanceAssistant()} disabled={loading || !assistantId}><ShieldCheck size={17} /> 新增考勤员</button>
        <div className="permission-note"><strong>考勤员可以做什么？</strong><span>填写本班考勤并查看统计；不能查看学员电话和备注，也不能管理课表、名册、小组、班级设置或导出数据。</span></div>
      </section>
    </div>
    <section className={`panel danger-zone ${currentClass.archived ? "restore-zone" : ""}`}><div><h2>{currentClass.archived ? "恢复班级" : "停用班级"}</h2><p>{currentClass.archived ? "恢复后可继续维护名册、课表和考勤。" : "停用不会删除历史记录；管理员和辅导员仍可查看，班长和考勤员立即停止访问。"}</p></div><button className={currentClass.archived ? "secondary" : "ghost danger"} onClick={() => void toggleClassStatus()} disabled={loading}><Archive size={17} /> {currentClass.archived ? "恢复正常使用" : "停用这个班级"}</button></section>
  </main>;
}
...
function AttendancePage({ currentClass, user }: { currentClass: ClassSummary; user: CurrentUser }) {
  ...
  const limitedAttendanceRole = currentClass.permission === "monitor" || currentClass.permission === "attendance_assistant";
  const editable = Boolean(payload?.canEdit) && !(payload?.lockedForMonitor && !user.isAdmin && limitedAttendanceRole);
...
function AppContent() {
  ...
  else if (path === "/lessons" && currentClass!.permission !== "attendance_assistant") content = <LessonsPage currentClass={currentClass!} isAdmin={user.isAdmin} onClassRefresh={() => loadClasses()} />;
