...
function LessonEditor({ classId, lesson, onClose, onSaved }: { classId: number; lesson: Lesson; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal title={`编辑第 ${lesson.lessonNumber} 课`} subtitle="修改未来课次日期会让后续未开始的安排整体顺延。" onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
      ...
      <label>班修 / 整课截止日<input type="date" value={classStudyDueDate} onChange={(e) => setClassDate(e.target.value)} required /><small>导图/提纲与组修日期会按本课学习模式自动计算；保存后，本课及后续未开始课次整体顺延。</small></label>
      <Notice notice={notice} />
      <div className="modal-actions"><button type="button" className="ghost" onClick={onClose}>取消</button><button className="primary" disabled={loading}>{loading ? "保存中..." : "保存并顺延"}</button></div>
    </form>
  </Modal>;
}
...
function GenerateSchedule({ classId, defaultMode, hasLessons, onClose, onSaved }: { classId: number; defaultMode: CadenceMode; hasLessons: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal title={hasLessons ? "追加学习课次" : "生成学习课表"} subtitle={hasLessons ? "从现有最后一课继续，按照班级当前学习模式排期。" : "设置第一课班修截止日，系统会按学习模式连续预排。"} onClose={onClose}>
    <form className="form-stack" onSubmit={submit}>
